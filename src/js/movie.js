...
const trailersHTML= videoKeys.map(videoKey => {
    ...
    return `
    <div class="relative cursor-pointer inline-block w-3/5 h-96" data-video-id="${videoKey}" onclick="loadVideo(this)">
  <img class="w-full h-full" src="https://img.youtube.com/vi/${videoKey}/maxresdefault.jpg" alt="Thumbnail">
...
fetch(`https://www.omdbapi.com/?i=${movie.imdb_id}&apikey=22b88799`)
  ...
  .then(data => {
    ...
    const movieTrailerHTML = `<ul class="slider-container">
    ${trailersHTML}
    </ul>`
    movieTrailerContainer.innerHTML = movieTrailerHTML;
